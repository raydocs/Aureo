...
export async function resolveWhisperExecutablePath(preferredPath?: string | null) {
	...
	throw new Error(
		"No Whisper runtime was found. Aureo looked for a bundled binary first, then checked common system install locations.",
	);
}
...
export async function generateAutoCaptionsFromVideo(options: {
	...
	const tempBase = path.join(
		app.getPath("temp"),
		`aureo-captions-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
	);
