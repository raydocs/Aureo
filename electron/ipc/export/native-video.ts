...
const NVIDIA_CUDA_EXPORT_ENV = "AUREO_EXPERIMENTAL_NVIDIA_CUDA_EXPORT";
const NVIDIA_CUDA_ALLOW_AUDIO_EXPORT_ENV = "AUREO_NVIDIA_CUDA_ALLOW_AUDIO_EXPORT";
const NVIDIA_CUDA_FORCE_VIDEO_ONLY_ENV = "AUREO_NVIDIA_CUDA_FORCE_VIDEO_ONLY";
const NVIDIA_CUDA_AUTO_STALL_TIMEOUT_ENV = "AUREO_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS";
const DEFAULT_NVIDIA_CUDA_AUTO_STALL_TIMEOUT_MS = 120_000;
const NATIVE_GPU_STALL_TIMEOUT_ENV = "AUREO_NATIVE_GPU_STALL_TIMEOUT_MS";
...
function shouldPersistNativeExportDiagnostics() {
	const rawValue = process.env.AUREO_NATIVE_EXPORT_DIAGNOSTICS?.trim().toLowerCase();
...
function shouldPersistNvidiaCudaExportDiagnostics() {
	return (
		...
		process.env.AUREO_NVIDIA_CUDA_EXPORT_DIAGNOSTICS === "1"
	);
}
...
async function resolveExperimentalWindowsGpuExporterPath() {
	...
	const executableNames = ["aureo-gpu-export.exe", "gpu-export-probe.exe"];
	const candidates: string[] = [];
	const configuredPath = process.env.AUREO_WINDOWS_GPU_EXPORT_EXE;
	if (configuredPath) {
...
export async function resolveExperimentalNvidiaCudaExportScriptPath() {
	...
	const candidates: string[] = [];
	const configuredPath = process.env.AUREO_NVIDIA_CUDA_EXPORT_SCRIPT;
	if (configuredPath) {
		candidates.push(configuredPath);
	}
	const resourcesPath = (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath;
...
function resolveExperimentalNvidiaCudaNodeCommand() {
	const configuredNodePath = process.env.AUREO_NVIDIA_CUDA_NODE_EXE;
...
export function buildExperimentalNvidiaCudaStaticLayoutArgs(
	...
	const args = [
		...
		"--stream-sync",
		"--prewarm-ms",
		process.env.AUREO_NVIDIA_CUDA_PREWARM_MS || "500",
		"--content-x",
		String(Math.round(options.offsetX)),
		"--content-y",
		String(Math.round(options.offsetY)),
	...
	if (process.env.AUREO_NVIDIA_CUDA_SAMPLE_GPU === "1") {
...
async function runExperimentalNvidiaCudaStaticLayoutExport(
	...
	const env = {
		...process.env,
		...nodeCommand.env,
		AUREO_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY:
			process.env.AUREO_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY ?? "1",
		AUREO_FFMPEG_EXE: ffmpegPath,
		AUREO_FFPROBE_EXE: process.env.AUREO_FFPROBE_EXE ?? getFfprobeBinaryPath(),
...
export async function exportNativeStaticLayoutVideo(
	...
	const sessionId =
		options.sessionId ??
		`aureo-static-layout-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export async function probeNativeVideoEncoder(
	...
	const outputPath = path.join(
		app.getPath("temp"),
		`aureo-export-probe-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
...
export async function muxNativeVideoExportAudio(
	...
	if (audioMode === "edited-track" && !useEditedTrackFiltergraph) {
		...
		audioInputPath = path.join(
			app.getPath("temp"),
			`aureo-export-audio-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${extension}`,
		);
...
export async function muxExportedVideoAudioBuffer(
	...
	const tempVideoPath = path.join(
		app.getPath("temp"),
		`aureo-export-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
	);
	const metrics: NativeVideoAudioMuxMetrics = {};
	let succeeded = false;
