...
export function getNativeCaptureHelperBinaryPath(): string {
	return path.join(app.getPath("userData"), "native-tools", "aureo-screencapturekit-helper");
}
...
export function getSystemCursorHelperBinaryPath(): string {
	return path.join(app.getPath("userData"), "native-tools", "aureo-system-cursors");
}
...
export function getNativeCursorMonitorBinaryPath(): string {
	return path.join(app.getPath("userData"), "native-tools", "aureo-native-cursor-monitor");
}
...
export function getNativeWindowListBinaryPath(): string {
	return path.join(app.getPath("userData"), "native-tools", "aureo-window-list");
}
...
export async function ensureNativeHelperMigration(): Promise<void> {
	// Hard-cut rename: no legacy helper name fallback/migration.
	if (!nativeHelperMigrationPromise) {
		setNativeHelperMigrationPromise(Promise.resolve());
	}
