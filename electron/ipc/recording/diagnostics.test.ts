...
describe("getCompanionAudioFallbackPaths", () => {
	...
	beforeEach(async () => {
		tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aureo-companion-audio-"));
	...
	it("keeps embedded system audio plus the mac mic sidecar when ScreenCaptureKit wrote both companions", async () => {
		const videoPath = path.join(tempRoot, "recording.mp4");
		...
		await Promise.all([
			...
			fs.writeFile(systemPath, "system"),
			fs.writeFile(micPath, "mic"),
			fs.writeFile(`${micPath}.json`, JSON.stringify({ startDelayMs: 420 })),
		]);
