...
function truncateDiagnosticsText(value: string | undefined, maxLength = 12000) {
	...
	return `${value.slice(-maxLength)}\n[aureo: truncated to last ${maxLength} chars]`;
}
...
export async function getCompanionAudioFallbackInfo(videoPath: string) {
	...
	if (await hasEmbeddedAudioStream(videoPath)) {
		...
		// Usable mac mic sidecars are tracked independently so ScreenCaptureKit
		// recordings (embedded system audio + .system.m4a + .mic.m4a) still keep
		// the dedicated mic track when a system companion is also present.
		const usableMacMicCompanions = Array.from(
			new Set(
		...
		if (!hasUsableMacSystemCompanion && usableMacMicOnlyCompanions.length > 0) {
			...
			// Embedded track already carries system audio; never re-add the system
			// sidecar, but do keep any usable mic companion(s).
			paths =
				usableMacMicCompanions.length > 0
					? [videoPath, ...usableMacMicCompanions]
					: [videoPath];
		} else {
