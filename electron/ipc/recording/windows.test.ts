...
describe("waitForWindowsCaptureStop", () => {
	...
	it("resolves the helper output path when the process closes cleanly", async () => {
		...
		setWindowsCaptureOutputBuffer("Recording stopped. Output path: C:\\Aureo\\capture.mp4");
		...
		await expect(stopped).resolves.toBe("C:\\Aureo\\capture.mp4");
	...
	it("resolves the fallback target path when the helper closes cleanly without output path", async () => {
		...
		setWindowsCaptureTargetPath("C:\\Aureo\\fallback.mp4");
		...
		await expect(stopped).resolves.toBe("C:\\Aureo\\fallback.mp4");
		expect(proc.kill).not.toHaveBeenCalled();
	});
