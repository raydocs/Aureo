	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or Aureo Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or Aureo Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "Aureo Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
