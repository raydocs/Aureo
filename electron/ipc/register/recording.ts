		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			if (process.platform === "win32") {
				...
				try {
					...
					tempVideoPath = path.join(app.getPath("temp"), `aureo-native-${timestamp}.mp4`);
					...
					if (options?.capturesSystemAudio) {
						...
						tempSystemAudioPath = path.join(
							app.getPath("temp"),
							`aureo-native-${timestamp}.system.wav`,
						);
					...
					if (options?.capturesMicrophone && !browserMicFallbackRequested) {
						...
						tempMicPath = path.join(app.getPath("temp"), `aureo-native-${timestamp}.mic.wav`);
			...
			try {
				...
				if (
					...
					appName &&
					(appName === ownAppName || appName === "aureo")
				) {
					return {
						success: false,
						message:
							"Cannot record Aureo windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Aureo needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Aureo is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Aureo needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Aureo is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
