...
// Runtime brand is Aureo. userData is pinned in appPaths.
app.setName("Aureo");
...
function getPlatformAppIconFilename(size: 32 | 128 | 512) {
	const baseName = process.platform === "darwin" ? "aureomac" : "aureo";
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Aureo ${payload.version} is available`;
		case "downloading":
			return `Downloading Aureo ${payload.version}`;
		case "ready":
			return `Aureo ${payload.version} is ready`;
		case "error":
			return `Aureo ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Aureo.";
		case "downloading":
			return "Aureo is downloading the update and will restart when it is ready.";
		case "ready":
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("dev.webadderall.aureo");
	}
	...
	if (IS_SMOKE_EXPORT || process.env.AUREO_DEV_OPEN_RECORDING_INPUT) {
		await logSmokeExportGpuDiagnostics();
		if (IS_SMOKE_EXPORT) {
			const smokeSource =
				process.env.AUREO_SMOKE_EXPORT_PROJECT ??
				process.env.AUREO_SMOKE_EXPORT_INPUT ??
				"<missing input>";
			console.log(`[smoke-export] Starting editor smoke export for ${smokeSource}`);
		} else {
			console.log(
				`[dev-open-recording] Starting editor for ${process.env.AUREO_DEV_OPEN_RECORDING_INPUT}`,
			);
		}
		createEditorWindowWrapper();
		return;
