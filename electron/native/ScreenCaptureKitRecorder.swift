...
final class ScreenCaptureRecorder: NSObject, SCStreamOutput, SCStreamDelegate {
	private let queue = DispatchQueue(label: "aureo.screencapturekit.video")
	...
	func startCapture(configJSON: String) async throws {
		guard !isRecording else {
			throw NSError(domain: "AureoCapture", code: 1, userInfo: [NSLocalizedDescriptionKey: "Recording is already in progress"])
		}

		guard let data = configJSON.data(using: .utf8) else {
			throw NSError(domain: "AureoCapture", code: 2, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON input"])
		}
		...
		if let windowId = config.windowId {
			...
			guard let window = availableContent.windows.first(where: { $0.windowID == windowId }) else {
				throw NSError(domain: "AureoCapture", code: 3, userInfo: [NSLocalizedDescriptionKey: "Window not found"])
			}
			...
			guard let display = availableContent.displays.first(where: { $0.displayID == displayId }) else {
				throw NSError(domain: "AureoCapture", code: 4, userInfo: [NSLocalizedDescriptionKey: "Display not found"])
			}
		...
		guard let assistant = AVOutputSettingsAssistant(preset: .preset3840x2160) else {
			throw NSError(domain: "AureoCapture", code: 5, userInfo: [NSLocalizedDescriptionKey: "Unable to create output settings assistant"])
		}
		...
		guard var outputSettings = assistant.videoSettings else {
			throw NSError(domain: "AureoCapture", code: 6, userInfo: [NSLocalizedDescriptionKey: "Output settings unavailable"])
		}
		...
		guard let assetWriter = assetWriter, assetWriter.canAdd(videoInput) else {
			throw NSError(domain: "AureoCapture", code: 7, userInfo: [NSLocalizedDescriptionKey: "Unable to add video writer input"])
		}
		...
		if writesSystemAudioToSeparateTrack {
			guard let systemAudioOutputPath = config.systemAudioOutputPath, !systemAudioOutputPath.isEmpty else {
				throw NSError(domain: "AureoCapture", code: 11, userInfo: [NSLocalizedDescriptionKey: "Missing system audio output path for audio capture"])
			}
			...
			guard systemAudioWriter.canAdd(systemAudioInput) else {
				throw NSError(domain: "AureoCapture", code: 12, userInfo: [NSLocalizedDescriptionKey: "Unable to add system audio writer input"])
			}
			...
			guard systemAudioWriter.startWriting() else {
				throw NSError(domain: "AureoCapture", code: 13, userInfo: [NSLocalizedDescriptionKey: systemAudioWriter.error?.localizedDescription ?? "Unable to start system audio writing"])
			}
		...
		if writesMicrophoneToSeparateTrack {
			guard let microphoneOutputPath = config.microphoneOutputPath, !microphoneOutputPath.isEmpty else {
				throw NSError(domain: "AureoCapture", code: 14, userInfo: [NSLocalizedDescriptionKey: "Missing microphone output path for microphone capture"])
			}
			...
			guard microphoneWriter.canAdd(microphoneInput) else {
				throw NSError(domain: "AureoCapture", code: 15, userInfo: [NSLocalizedDescriptionKey: "Unable to add microphone writer input"])
			}
			...
			guard microphoneWriter.startWriting() else {
				throw NSError(domain: "AureoCapture", code: 16, userInfo: [NSLocalizedDescriptionKey: microphoneWriter.error?.localizedDescription ?? "Unable to start microphone audio writing"])
			}
		...
		guard assetWriter.startWriting() else {
			throw NSError(domain: "AureoCapture", code: 8, userInfo: [NSLocalizedDescriptionKey: assetWriter.error?.localizedDescription ?? "Unable to start video writing"])
		}
	...
	func stopCapture() async throws -> String {
		guard isRecording else {
			throw NSError(domain: "AureoCapture", code: 9, userInfo: [NSLocalizedDescriptionKey: "No recording in progress"])
		}
...
final class RecorderService {
	...
	private let queue = DispatchQueue(label: "aureo.screencapturekit.commands")
