...
const ffmpegCommand = resolveToolCommand(["AUREO_FFMPEG_EXE"], "ffmpeg-static", "ffmpeg");
const ffprobeCommand = resolveToolCommand(["AUREO_FFPROBE_EXE"], "ffprobe-static", "ffprobe");
...
function shouldRaiseChildPriority() {
	return process.env.AUREO_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY !== "0";
}
...
const outputPath = resolve(
	getArg("--output", join(scriptDir, "aureo-nvdec-nvenc-mp4-output.mp4")),
);
...
function resolveNativeProbePath() {
	const configuredPath = process.env.AUREO_NVIDIA_CUDA_EXPORT_EXE;
	const platformArch = process.arch === "arm64" ? "win32-arm64" : "win32-x64";
	const candidates = [
		configuredPath,
		join(scriptDir, "build", "Release", "aureo-nvidia-cuda-compositor.exe"),
		join(
			repoRoot,
			"electron",
			"native",
			"bin",
			platformArch,
			"aureo-nvidia-cuda-compositor.exe",
		),
	].filter(Boolean);
