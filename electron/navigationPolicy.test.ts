...
describe("isInternalRendererTarget", () => {
	it.each([
		...
		[
			"file:///opt/Aureo/dist/index.html?windowType=editor",
			"file:///opt/Aureo/dist/index.html?windowType=hud-overlay#status",
		],
	...
	it.each([
		...
		["https://aureo.example/", "http://aureo.example/"],
		["https://aureo.example/", "https://user:pass@aureo.example/"],
		["file:///opt/Aureo/dist/index.html", "file:///etc/passwd"],
		["file:///opt/Aureo/dist/index.html", "data:text/html,hello"],
...
describe("navigation event handlers", () => {
	...
	it("stops same-file query mutation without externalizing it", () => {
		...
		handler({
			url: "file:///opt/Aureo/dist/index.html?smokeExport=1",
			preventDefault,
		});
	...
	it("stops unsafe schemes without opening them externally", () => {
		...
		const openExternal = vi.fn(async () => undefined);
		const handler = createWillNavigateHandler(
			() => "file:///opt/Aureo/dist/index.html",
			openExternal,
		);
	...
	it("does not trust a renderer-mutated URL as an exact reload", () => {
		let currentUrl = "file:///opt/Aureo/dist/index.html?windowType=editor";
		const on = vi.fn();
		const webContents = {
			getURL: () => currentUrl,
			on,
		...
		currentUrl = "file:///opt/Aureo/dist/index.html?windowType=source-selector";
	...
	it("trusts an exact reload after a completed document navigation", () => {
		...
		const loadedUrl = "file:///opt/Aureo/dist/index.html?windowType=editor";
