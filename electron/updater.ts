...
const AUTO_UPDATES_DISABLED = process.env.AUREO_DISABLE_AUTO_UPDATES === "1";
const UPDATE_FEED_URL_OVERRIDE = process.env.AUREO_UPDATE_FEED_URL?.trim() ?? "";
const UPDATER_LOG_PATH =
	process.env.AUREO_UPDATER_LOG_PATH?.trim() || path.join(USER_DATA_PATH, "updater.log");
...
function configureUpdateFeed() {
	if (!UPDATE_FEED_URL_OVERRIDE) {
		writeUpdaterLog("Auto-update feed is not configured; updater remains disabled.");
		return false;
	}
	...
	writeUpdaterLog(`Using configured update feed: ${UPDATE_FEED_URL_OVERRIDE}`);
	return true;
}
...
export function isAutoUpdateFeatureEnabled() {
	return !AUTO_UPDATES_DISABLED && Boolean(UPDATE_FEED_URL_OVERRIDE);
}
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Aureo will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Aureo restarts.`
					: "Downloading the update now. Aureo will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Aureo ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Aureo ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Aureo ${version} is ready to install.`
			: `Aureo ${version} has been downloaded.`,
		detail: isPreview
...
export async function checkForAppUpdates(
	...
	if (!canUseAutoUpdates()) {
		writeUpdaterLog(
			`Skipped update check because auto-updates are unavailable. packaged=${app.isPackaged} mas=${process.mas ? "yes" : "no"} disabled=${AUTO_UPDATES_DISABLED ? "yes" : "no"} feedConfigured=${UPDATE_FEED_URL_OVERRIDE ? "yes" : "no"}`,
		);
		if (options?.manual) {
			await showMessageBox(getMainWindow, {
				type: "info",
				...
				detail:
					"Aureo does not currently publish an update feed. Automatic updates stay off until an update feed is explicitly configured.",
			});
			setUpdateStatusSummary({
				status: "idle",
				availableVersion: null,
				detail: "Updates are temporarily unavailable.",
			});
		}
		return;
	}
...
export function setupAutoUpdates(
	...
	if (!canUseAutoUpdates()) {
		...
		writeUpdaterLog(
			`Auto-updates disabled by default. Set AUREO_UPDATE_FEED_URL to enable a feed. logPath=${UPDATER_LOG_PATH}`,
		);
		return;
	}
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Aureo ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Aureo ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Aureo ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
