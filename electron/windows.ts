...
function getEditorWindowQuery(): Record<string, string> {
	...
	if (process.env.AUREO_DEV_OPEN_RECORDING_WEBCAM) {
		query.devOpenWebcam = process.env.AUREO_DEV_OPEN_RECORDING_WEBCAM;
	}

	if (process.env.AUREO_SMOKE_EXPORT === "1") {
		query.smokeExport = "1";
		if (process.env.AUREO_SMOKE_EXPORT_INPUT) {
			query.smokeInput = process.env.AUREO_SMOKE_EXPORT_INPUT;
		}
		if (process.env.AUREO_SMOKE_EXPORT_OUTPUT) {
			query.smokeOutput = process.env.AUREO_SMOKE_EXPORT_OUTPUT;
		}
		if (process.env.AUREO_SMOKE_EXPORT_USE_NATIVE === "1") {
			query.smokeUseNativeExport = "1";
		}
		if (process.env.AUREO_SMOKE_EXPORT_ENCODING_MODE) {
			query.smokeEncodingMode = process.env.AUREO_SMOKE_EXPORT_ENCODING_MODE;
		}
		if (process.env.AUREO_SMOKE_EXPORT_SHADOW_INTENSITY) {
			query.smokeShadowIntensity = process.env.AUREO_SMOKE_EXPORT_SHADOW_INTENSITY;
		}
		if (process.env.AUREO_SMOKE_EXPORT_WEBCAM_INPUT) {
			query.smokeWebcamInput = process.env.AUREO_SMOKE_EXPORT_WEBCAM_INPUT;
		}
		if (process.env.AUREO_SMOKE_EXPORT_WEBCAM_SHADOW) {
			query.smokeWebcamShadow = process.env.AUREO_SMOKE_EXPORT_WEBCAM_SHADOW;
		}
		if (process.env.AUREO_SMOKE_EXPORT_WEBCAM_SIZE) {
			query.smokeWebcamSize = process.env.AUREO_SMOKE_EXPORT_WEBCAM_SIZE;
		}
		if (process.env.AUREO_SMOKE_EXPORT_PIPELINE) {
			query.smokePipelineModel = process.env.AUREO_SMOKE_EXPORT_PIPELINE;
		}
		if (process.env.AUREO_SMOKE_EXPORT_BACKEND) {
			query.smokeBackendPreference = process.env.AUREO_SMOKE_EXPORT_BACKEND;
		}
		if (process.env.AUREO_SMOKE_EXPORT_RENDER_BACKEND) {
			query.smokeRenderBackend = process.env.AUREO_SMOKE_EXPORT_RENDER_BACKEND;
		}
		if (process.env.AUREO_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
			query.smokeMaxEncodeQueue = process.env.AUREO_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
		}
		if (process.env.AUREO_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
			query.smokeMaxDecodeQueue = process.env.AUREO_SMOKE_EXPORT_MAX_DECODE_QUEUE;
		}
		if (process.env.AUREO_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
			query.smokeMaxPendingFrames = process.env.AUREO_SMOKE_EXPORT_MAX_PENDING_FRAMES;
		}
		if (process.env.AUREO_SMOKE_EXPORT_PROJECT) {
			query.smokeProject = process.env.AUREO_SMOKE_EXPORT_PROJECT;
		}
		if (process.env.AUREO_SMOKE_EXPORT_QUALITY) {
			query.smokeQuality = process.env.AUREO_SMOKE_EXPORT_QUALITY;
		}
		if (process.env.AUREO_SMOKE_EXPORT_FPS) {
			query.smokeFps = process.env.AUREO_SMOKE_EXPORT_FPS;
		}
	}
