...
const bundledExePath = path.join(bundledDir, "aureo-nvidia-cuda-compositor.exe");
const helperId = "aureo-nvidia-cuda-compositor";
const generatorArch = process.arch === "arm64" ? "ARM64" : "x64";
const videoCodecSdkRoot =
	process.env.AUREO_NVIDIA_VIDEO_CODEC_SDK_ROOT?.trim() ||
...
function fallbackToBundledHelperOrExit(reason) {
	if (existsSync(bundledExePath)) {
		const verification = verifyNativeHelperManifest({
			...
			binaryName: "aureo-nvidia-cuda-compositor.exe",
		});
		if (!verification.ok) {
			console.warn(
...
if (!existsSync(path.join(videoCodecSdkRoot, "Samples", "NvCodec"))) {
	fallbackToBundledHelperOrExit(
		`NVIDIA Video Codec SDK samples not found at ${videoCodecSdkRoot}. Set AUREO_NVIDIA_VIDEO_CODEC_SDK_ROOT to build from source.`,
	);
}
...
function patchNvDecoderForAureoCallbacks() {
	...
	if (!header.includes("AureoMappedFrameHandler")) {
...
using AureoMappedFrameHandler = void (*)(CUdeviceptr, unsigned int, int, int, int, int64_t, void*);
using AureoDisplayFramePolicy = bool (*)(int, void*);
`,
    ...
    void SetMappedFrameHandler(AureoMappedFrameHandler handler, void* userData) { m_aureoMappedFrameHandler = handler; m_aureoMappedFrameUserData = userData; }
    void SetDisplayFramePolicy(AureoDisplayFramePolicy policy, void* userData) { m_aureoDisplayFramePolicy = policy; m_aureoDisplayFramePolicyUserData = userData; }
...
`,
    ...
    AureoMappedFrameHandler m_aureoMappedFrameHandler = nullptr;
    void* m_aureoMappedFrameUserData = nullptr;
    AureoDisplayFramePolicy m_aureoDisplayFramePolicy = nullptr;
    void* m_aureoDisplayFramePolicyUserData = nullptr;
`,
	...
	if (!source.includes("Aureo mapped frame callback")) {
    ...
    if (m_aureoDisplayFramePolicy &&
        !m_aureoDisplayFramePolicy(displayFrameIndex, m_aureoDisplayFramePolicyUserData))
    {
        NVDEC_API_CALL(cuvidUnmapVideoFrame(m_hDecoder, dpSrcFrame));
        return 1;
    }

    // Aureo mapped frame callback keeps the CUDA helper from making an
    // extra device-to-device copy when the caller can consume mapped NV12.
    if (m_aureoMappedFrameHandler)
    {
        m_aureoMappedFrameHandler(
            dpSrcFrame,
            nSrcPitch,
            m_nWidth,
            m_nHeight,
...
try {
	patchNvDecoderForAureoCallbacks();
} catch (error) {
	fallbackToBundledHelperOrExit(
...
try {
	configureWithWindowsCmakeGenerator({
		...
		configure: (generator, toolset) =>
			execSync(
				`${cmake} .. -G "${generator}" -A ${generatorArch}${toolset ? ` -T ${toolset}` : ""} -DAUREO_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
				{
					cwd: buildDir,
					stdio: "inherit",
					timeout: 120000,
...
const exePath = path.join(buildDir, "Release", "aureo-nvidia-cuda-compositor.exe");
